...
import com.tomorrowmust.system.common.RedisIdWorker;
import com.tomorrowmust.system.domain.dto.BookingMessage;
...
import com.tomorrowmust.system.mapper.TBookingOrderMapper;
...
import static com.tomorrowmust.system.domain.constant.MqConstant.BOOKING_TOPIC;
...
public class TBookingOrderServiceImpl extends ServiceImpl<TBookingOrderMapper, TBookingOrder> implements ITBookingOrderService {
    ...
    @PostConstruct
    public void saveOrderToRedis() {
        List<TBookingOrder> orders = query().list();
        stringRedisTemplate.executePipelined((RedisCallback<Object>) connection -> {
            for (TBookingOrder order : orders) {
                String key = "booking:order:" + order.getStockId();
                String value = order.getUserId().toString();
    ...
    @Override
    public Result saveBookingOrder(Long stockId,Long userId) {
    ...
    private void rollbackRedis(Long stockId, Long userId) {
        stringRedisTemplate.opsForValue().increment("order:stock:" + stockId);
        stringRedisTemplate.opsForSet().remove("booking:order:" + stockId, userId.toString());
    }
    ...
    public Result queryMyBooking(Integer current) {
        ...
        Page<TBookingOrder> page = new Page<>(current, 5);
    ...
    @Override
    @Transactional
    public Result cancelBooking(Long id, Long userId) {
    ...
    private void cancelOrder(TBookingOrder order, Long userId) {
        // 更新订单状态
        ...
        stringRedisTemplate.delete("order:stock:" + stockId);
        // 如果有用户预约状态的缓存也需要清理
        stringRedisTemplate.opsForSet().remove("booking:order:" + stockId, userId.toString());
    }
